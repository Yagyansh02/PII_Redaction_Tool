...
The included `Dockerfile` deploys both UI and API as a Docker web service. `requirements-cloud.txt` installs `en_core_web_lg`, the same model as the reference `requirements.txt` environment, so NER precision/recall does not diverge between local and deployed runs; earlier deployments pinned the smaller `en_core_web_sm` to fit a 512 MB instance, but this required different-behavior tradeoffs from local testing and was moved off. The host must be sized for the large model: budget ~1.5-2 GB RAM at model-load time (plus ~560 MB image size for the model artifacts) and monitor actual peak memory, which still depends on document size. Use an access-controlled paid deployment after completing a privacy/security review; the service is not intended to hold confidential production data on a free/shared tier.
